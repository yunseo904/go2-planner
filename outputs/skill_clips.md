...
`skill_clips.npz` — sha256 `aab85a03f5d90635…`, 4 clips.
...
**Downsampling is nearly free for `WALK`, `TROT`.** The sport controller wrote new commands at ~43 Hz on those sessions, below the 50 Hz target, so the ~419 Hz copy is a zero-order-held staircase of the same information. `RUN` and `JUMP` update in the hundreds of Hz and do lose content.
