...
**기본은 GPU 없이 돈다.** `scripts/isaac_docker_run.sh` 의 기본값은 `GPU=none` 이고,
스크립트는 `--device cpu` 로 돌린다.

- **측정용 리플레이는 GPU가 필요 없다 (실측).** 2026-08-29, TROT `--foot-comp raibert
  --foot-clip-rad 0.05` 60사이클을 컨테이너에 `/dev/nvidia*` 가 하나도 없는 상태로 돌려
  GPU 런과 결과 행의 **모든 컬럼이 비트 단위로 일치**했고 벽시계 시간도 36초로 같았다.
  물리는 CPU, 헤드리스 Kit은 렌더를 하지 않으므로 카드는 놀고 있었다. 확인 전에 스윕
  한 판을 남의 GPU를 세워 가며 돌린 적이 있다 — 다시 하지 않는다.
- **GPU가 실제로 필요한 것은 렌더링 두 가지뿐이다**: `--enable_cameras`/`--video`
  (측면 영상), `LIVE=1` (WebRTC 스트림). 카드 없이 돌리면 실패하지 않고
  `carb.cudainterop ... cudaErrorInsufficientDriver` 를 무한히 뿜으며 **멈춘다**.
- **GPU1 을 빌려야 하면 먼저 사용자에게 알린다.** 빌리는 절차 자체는
  `~/WMP_중단_재개.md` 에 있고 그대로 따르되, 실행 전에 왜 필요한지 말하고 승인을 받는다.
  남의 학습을 세우는 일이므로 자주 할 수 없다.
- **GPU0 은 어떤 경우에도 쓰지 않는다.**
